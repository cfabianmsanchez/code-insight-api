...
class ResourcePromptProviderTest {
    ...
    void systemPrompt_shouldLoadVersionedSystemPromptTemplate() {
        ...
        assertThat(systemPrompt).contains("Ingeniero de Software Senior y Arquitecto de Soluciones");
        assertThat(systemPrompt).contains("PRINCIPIO CRÍTICO: La ausencia de una evidencia NO implica");
    }
    ...
    void analysisDirectives_shouldLoadVersionedDirectivesTemplate() {
        ...
        assertThat(directives).contains("Directivas para la Síntesis Arquitectónica");
    ...
    void init_shouldThrowExceptionWhenPromptVersionNotFound() {
        ResourcePromptProvider invalidProvider = new ResourcePromptProvider(new DefaultResourceLoader(), "v999");

        IllegalStateException exception = assertThrows(IllegalStateException.class, invalidProvider::init);
        assertThat(exception.getMessage()).contains("No se encontró el artefacto de prompt");
    }
}
