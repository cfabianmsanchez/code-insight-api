...
public class OllamaAdapter
  implements ArchitectureSynthesisPort, AiModelManagementPort
{
  ...
  public OllamaAdapter(
    ...
    this(
      ...
      baseUrl,
      model,
      "qwen2.5-coder,llama3,deepseek-coder,codellama,mistral"
    );
  }
  ...
  public OllamaAdapter(
    ...
    @Value(
      "${ollama.available-models:qwen2.5-coder,llama3,deepseek-coder,codellama,mistral}"
  ...
  public void setActiveModel(String model) {
    if (model == null || model.isBlank()) {
      throw new IllegalArgumentException(
        "El nombre del modelo de IA no puede estar vacío."
      );
    }
    ...
    if (!available.contains(cleanModel)) {
      throw new IllegalArgumentException(
        "El modelo de Ollama no está instalado o disponible en el servidor: " +
          cleanModel
      );
    }
    this.activeModel.set(cleanModel);
  ...
  public List<String> fetchAvailableModels() {
    try {
      OllamaTagsResponse tagsResponse = restClient
        .get()
        .uri("/api/tags")
        .retrieve()
      ...
      log.debug(
        "No se pudieron obtener los modelos dinámicos de Ollama en /api/tags: {}",
        e.getMessage()
      );
    }
  ...
  public String synthesize(String systemPrompt, String userPrompt) {
    ...
    try {
      ...
      log.info(
        "Enviando prompt de análisis a Ollama en {} con el modelo {}",
        baseUrl,
        currentModel
      );
      ...
      log.warn(
        "Respuesta nula o sin contenido recibida de Ollama. Se recurre al reporte de respaldo."
      );
      return buildFallbackReport("Respuesta vacía o nula del servidor Ollama.");
    } catch (Exception e) {
      log.warn(
        "No se pudo conectar con el servidor Ollama en {}. Detalle: {}. Se utilizará el reporte de respaldo.",
        baseUrl,
        e.getMessage()
      );
