...
@CrossOrigin(origins = "*")
@Tag(name = "System Configuration", description = "Endpoint for inspecting and updating runtime backend configuration")
public class SystemConfigController {
     ...
     */
    @GetMapping
    @Operation(summary = "Get active system configuration", description = "Returns active Ollama LLM model name, available models, base URL, and prompt version.")
    public ResponseEntity<SystemConfigResponseDto> getSystemConfig() {
        return ResponseEntity.ok(new SystemConfigResponseDto(
     ...
     */
    @PutMapping("/model")
    @Operation(summary = "Update active LLM model", description = "Updates the active Ollama LLM model used for architecture synthesis.")
    public ResponseEntity<SystemConfigResponseDto> updateActiveModel(@Valid @RequestBody UpdateModelRequestDto request) {
