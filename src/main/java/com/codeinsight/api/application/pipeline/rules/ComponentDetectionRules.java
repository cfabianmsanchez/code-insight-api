...
public final class ComponentDetectionRules {
  ...
  private static final List<ComponentRule> JAVA_RULES = List.of(
    ...
    new ComponentRule(
      c ->
        hasAnnotation(c, "Repository") ||
        hasAnyInterfaceExtension(c, "JpaRepository", "CrudRepository"),
      ComponentType.REPOSITORY
    ),
    new ComponentRule(
      c -> hasAnyAnnotation(c, "Entity", "Table"),
      ComponentType.ENTITY
    ),
    new ComponentRule(
      c -> hasAnnotation(c, "Configuration"),
      ComponentType.CONFIGURATION
    ),
    new ComponentRule(
      c -> hasAnnotation(c, "Component"),
  ...
  private static final List<ComponentRule> TS_JS_RULES = List.of(
    ...
    new ComponentRule(
      c -> hasAnnotation(c, "Injectable"),
      ComponentType.FRONTEND_SERVICE
    ),
    new ComponentRule(
      c -> hasAnnotation(c, "Module"),
      ComponentType.CONFIGURATION
    ),
    new ComponentRule(
      c -> hasAnyAnnotation(c, "Entity", "Table"),
  ...
  private static final List<ComponentRule> PYTHON_RULES = List.of(
    ...
    new ComponentRule(
      c -> containsAny(c, "models.model", "declarative_base()", "column("),
      ComponentType.ENTITY
    )
  );
   ...
   */
  public static final Map<String, List<ComponentRule>> RULES_BY_EXTENSION =
    Map.of(
      ".java",
      JAVA_RULES,
      ".ts",
  ...
  static boolean hasAnnotation(String content, String annotation) {
    return Pattern.compile(
      "@" + Pattern.quote(annotation.toLowerCase()) + "\\b"
    )
      .matcher(content)
      .find();
  }
  ...
  static boolean hasInterfaceExtension(String content, String interfaceName) {
    return Pattern.compile(
      "\\bextends\\s+.*" + Pattern.quote(interfaceName.toLowerCase()) + "\\b"
    )
      .matcher(content)
      .find();
  }

  static boolean hasAnyInterfaceExtension(
    String content,
