...
public class ContextBuilderStage {
  ...
  public AnalysisContext buildContext(
    ...
    userPromptBuilder.append(
      "# Radiografía de Ingeniería Inversa del Repositorio\n\n"
    );

    // 0. Metadata del Proyecto (extraída de manifiestos: package.json, etc.)
    EngineeringEvidence engMeta =
      architectureEvidence != null
        ? architectureEvidence.getEngineeringEvidence()
        : null;
    if (
      engMeta != null &&
      (engMeta.projectName() != null ||
        engMeta.projectDescription() != null ||
        engMeta.mainEntry() != null)
    ) {
      userPromptBuilder.append(
        "## 0. Metadata del Proyecto (extraída del manifiesto)\n"
      );
      if (engMeta.projectName() != null) userPromptBuilder
        .append("- **Nombre del Proyecto**: ")
        .append(engMeta.projectName())
        .append("\n");
      if (engMeta.projectDescription() != null) userPromptBuilder
        .append("- **Descripción Declarada**: ")
        .append(engMeta.projectDescription())
        .append("\n");
      if (engMeta.mainEntry() != null) userPromptBuilder
        .append("- **Punto de Entrada Principal**: ")
        .append(engMeta.mainEntry())
        .append("\n");
      if (engMeta.testScript() != null) userPromptBuilder
        .append("- **Script de Test**: `")
        .append(engMeta.testScript())
        .append("`\n");
      userPromptBuilder.append("\n");
    }
    ...
    if (request != null) {
      userPromptBuilder
        .append("- **Project Key**: ")
        .append(request.getProjectKey())
        .append("\n");
      userPromptBuilder
        .append("- **Fuente**: ")
        .append(request.getSourceType())
        .append("\n");
    }
    if (scannedFiles != null) {
      userPromptBuilder
        .append("- **Total de Archivos**: ")
        .append(scannedFiles.getTotalFiles())
        .append("\n");
      userPromptBuilder
        .append("- **Total de Directorios**: ")
        .append(scannedFiles.getTotalDirectories())
        .append("\n");
      userPromptBuilder
        .append("- **Conteo por Extensión**: ")
        .append(scannedFiles.getExtensionCounts())
        .append("\n");
    }
    userPromptBuilder.append("\n");
    ...
    if (techStack != null) {
      userPromptBuilder
        .append("- **Lenguaje Principal**: ")
        .append(techStack.getMainLanguage())
        .append("\n");
      userPromptBuilder
        .append("- **Framework Principal**: ")
        .append(techStack.getMainFramework())
        .append("\n");
      userPromptBuilder
        .append("- **Herramienta de Compilación**: ")
        .append(techStack.getBuildTool())
        .append("\n");
      userPromptBuilder
        .append("- **Bases de Datos Detectadas**: ")
        .append(techStack.getDatabasesDetected())
        .append("\n");
      userPromptBuilder
        .append("- **Librerías Clave**: ")
        .append(techStack.getKeyLibraries())
        .append("\n");
    }
    userPromptBuilder.append("\n");
    ...
    if (componentAnalysis != null) {
      userPromptBuilder
        .append("- **Total de Componentes**: ")
        .append(componentAnalysis.getTotalComponents())
        .append("\n");
      userPromptBuilder
        .append("- **Distribución por Estereotipo**: ")
        .append(componentAnalysis.getComponentCounts())
        .append("\n");
      userPromptBuilder.append(
        "- **Lista de Componentes y Rutas de Evidencia**:\n"
      );
    ...
    userPromptBuilder.append(
      "## 4. Evidencias y Señales Estructurales de Arquitectura\n"
    );
    if (architectureEvidence != null) {
      userPromptBuilder
        .append("- **Rutas Estructurales Distintas**: ")
        .append(architectureEvidence.getTotalStructuralPaths())
        .append("\n");
      userPromptBuilder
        .append("- **Profundidad Máxima de Rutas**: ")
        .append(architectureEvidence.getMaxPathDepth())
        .append(" segmentos\n");
      userPromptBuilder
        .append("- **Palabras Clave de Arquitectura Encontradas**: ")
        .append(architectureEvidence.getDetectedKeywords())
        .append("\n");
      userPromptBuilder
        .append("- **Distribución de Componentes por Paquete/Capa**: ")
        .append(architectureEvidence.getPackageComponentDistribution())
        .append("\n");
      ...
      if (!architectureEvidence.getInboundPortImplementations().isEmpty()) {
        ...
        architectureEvidence
          .getInboundPortImplementations()
          .forEach((port, impl) ->
            userPromptBuilder
              .append("  - `")
              .append(impl)
              .append("` implementa puerto inbound `")
              .append(port)
              .append("`\n")
          );
      }
      ...
      if (!architectureEvidence.getOutboundAdapterImplementations().isEmpty()) {
        ...
        userPromptBuilder.append(
          "Los siguientes adaptadores de infraestructura implementan puertos de salida:\n"
        );
        architectureEvidence
          .getOutboundAdapterImplementations()
          .forEach((adapter, port) ->
            userPromptBuilder
              .append("  - `")
              .append(adapter)
              .append("` implementa puerto outbound `")
              .append(port)
              .append("`\n")
          );
      }
      ...
      if (eng != null) {
        ...
        userPromptBuilder
          .append(
            "IMPORTANTE: Estas evidencias confirman prácticas presentes. "
          )
          .append(
            "La ausencia de una evidencia aquí NO implica que la práctica no exista.\n"
          );

        // Genérico: tests multiplataforma
        userPromptBuilder
          .append("- **Archivos de Test Detectados (multiplataforma)**: ")
          .append(eng.testFilesDetected())
          .append("\n");
        if (!eng.testDirectories().isEmpty()) {
          userPromptBuilder
            .append("- **Directorios de Test Encontrados**: ")
            .append(eng.testDirectories())
            .append("\n");
        }
        if (eng.testScriptDetected()) {
          userPromptBuilder
            .append("- **Script de Test (package.json)**: `")
            .append(eng.testScript())
            .append("`\n");
        }

        // Spring/Java específico — solo mostrar si hay evidencia relevante
        if (
          eng.springConfigurationDetected() ||
          eng.beanDefinitions() > 0 ||
          eng.constructorInjectionDetected()
        ) {
          userPromptBuilder
            .append("- **Inyección de Dependencias Spring (@Configuration)**: ")
            .append(
              eng.springConfigurationDetected() ? "DETECTADA" : "No observada"
            )
            .append("\n");
          userPromptBuilder
            .append("- **Definiciones @Bean**: ")
            .append(eng.beanDefinitions())
            .append("\n");
          userPromptBuilder
            .append("- **Inyección por Constructor**: ")
            .append(
              eng.constructorInjectionDetected() ? "DETECTADA" : "No observada"
            )
            .append("\n");
        }
      }
      ...
      userPromptBuilder
        .append("- **Tipo de Proyecto Inferido (ProjectKind)**: ")
        .append(architectureEvidence.getProjectKind())
        .append("\n");
      if (
        architectureEvidence.getFrontendFramework() != null &&
        architectureEvidence.getFrontendFramework() !=
          com.codeinsight.api.domain.model.FrontendFramework.NONE
      ) {
        userPromptBuilder
          .append("- **Framework Frontend Detectado**: ")
          .append(architectureEvidence.getFrontendFramework())
          .append("\n");
      }
      if (
        architectureEvidence.getFrontendEvidenceNotes() != null &&
