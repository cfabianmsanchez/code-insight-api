...
public final class TechnologyDetectionRules {
    ...
    public static final Map<String, ManifestRules> MANIFEST_RULES_BY_FILE = Map.of(
            ...
            "dockerfile", new ManifestRules(
                    ...
                    List.of(),
                    List.of(
                            new LibraryRule(List.of("from "), "Docker Containerization")
                    )
            ),

            "main.tf", new ManifestRules(
                    "Terraform CLI",
                    List.of(
                            new FrameworkRule(List.of("resource", "module", "terraform", "provider"), "Terraform IaC", "HCL / Terraform")
                    ),
                    List.of(),
                    List.of(
                            new LibraryRule(List.of("provider \"aws\"", "aws_"), "AWS Provider"),
                            new LibraryRule(List.of("provider \"azurerm\"", "azurerm_"), "Azure Provider"),
                            new LibraryRule(List.of("provider \"google\"", "google_"), "GCP Provider"),
                            new LibraryRule(List.of("provider \"kubernetes\"", "kubernetes_"), "Kubernetes Provider")
                    )
            )
    );
